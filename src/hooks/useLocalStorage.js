...
import { useState} from 'react';
 ...
 export const useLocalStorage = (key, initalValue) => { //key must be set to a string, initalValue can be anything
    const [storedValue, setStoredValue] = useState(() => {
        ...
        return item ? JSON.parse(item) : initalValue; // if we have key stored in local storage return it and parse it,
                   console.log("Current", item);                                   // if not return initalValue
             })
